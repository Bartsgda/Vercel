(function () {
  ...
  const reader = window.reader = {
    ...
    queue: [],
    prepared: false,
    audio: null, // Native audio object
    ...
    prepare() {
      ...
      if (window.CHAPTER_AUDIO) {
        this.audio = new Audio(window.CHAPTER_AUDIO);
    ...
    startFrom(idx) {
      this.prepare();
      if (this.audio) {
        // If we have real audio, dblclick might not work perfectly without timestamps
        // So we just play from beginning or current position for now
  ...
  if (speed) {
    speed.addEventListener("input", e => {
      ...
      if (reader.audio) reader.audio.playbackRate = reader.rate;
